...
- `-g, --generate-completions <SHELL>` produces a shell completion script for `bash`, `zsh`, `fish`, `powershell` or `elvish` and exits — install it wherever your shell expects completion files (e.g. `~/.local/share/bash-completion/completions/uv-prune`).
